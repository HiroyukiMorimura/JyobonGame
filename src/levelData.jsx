/* eslint-disable react-refresh/only-export-components */
// src/levelData.js

// タイルID
export const TILE_ID = { Empty: 0, Ground: 1, Spike: 2, Flag: 3, Platform: 4 };

// レジェンド: . 空白 / # 地面 / ^ トゲ / F ゴール旗
export const LEGEND = {
  ".": TILE_ID.Empty,
  "#": TILE_ID.Ground,
  "^": TILE_ID.Spike,
  F: TILE_ID.Flag,
};

/**
 * タイルサイズ (SideScrollerGame.jsxのTILEと同じ値)
 */
export const TILE = 48;

/**
 * ピクセル座標への変換ヘルパー関数
 * @param {number} n タイル数
 * @returns {number} ピクセル座標
 */
export const T = (n) => n * TILE;

// =================================================================
// 🌟 意地悪なステージ定義 (約300タイル幅) 🌟
// スタート直後にトゲ、高速移動床、精密ジャンプエリアを配置
// =================================================================
export const LEVEL = String.raw`
........................................................................................................................
........................................................................................................................
........................................................................................................................
....................###..............^^.............####..............##^##..##......#####...........................
...............###...............##......##..................#######..................##.....##.........................
..............###.............####........####....#^^^^^..................^^^^^.....###.....###......................F#.
############..###.............#.....................##.......................##.....###.....###.......................#.
############..###.....####....#..............####...##.....####.....####.....##....####.....###.....####.....####.....#.
############..###.....#..#....#..............#..#..........#..#.....#..#............###.....###.....#..#.....#..#.....#.
############..###.....#######r################..############..#######..###############..#####..#######..#######..#######.
############..##############################################################################################..##########.
############..##############################################################################################..##########.
`;

// =================================================================
// 🌟 可動ギミック（高速化・複雑化）🌟
// =================================================================

// 移動床 (x, y はピクセル座標, w はピクセル幅)
export const initialMovers = [
  // 意地悪1: スタート直後の超高速ピストン床（猶予なし）
  { x: T(15), y: T(6), w: T(2), left: T(3), right: T(19), vx: 3.5 },
  // 意地悪2: トゲの海の上の高速・長距離スライド床
  { x: T(35), y: T(7), w: T(3), left: T(30), right: T(50), vx: -2.0 },
  // 意地悪3: 2つの床が上下逆の動きをする（すれ違いを避けさせる）
  { x: T(80), y: T(6), w: T(2), left: T(78), right: T(85), vx: 1.5 },
  { x: T(90), y: T(7), w: T(2), left: T(88), right: T(95), vx: -1.5 },
  // 意地悪4: 長距離を高速で往復する細い足場
  { x: T(150), y: T(5), w: T(1), left: T(140), right: T(160), vx: 2.2 },
];

// 敵（高速化・狭い範囲を巡回）
export const initialEnemies = [
  // 意地悪1: 最初の床を降りた後の低空高速敵
  {
    x: T(25),
    y: T(7) - 24,
    w: 36,
    h: 36,
    left: T(15),
    right: T(30),
    vx: 3.0,
  },
  // 意地悪2: 狭い空間を高速巡回
  {
    x: T(65),
    y: T(8) - 24,
    w: 36,
    h: 36,
    left: T(63),
    right: T(70),
    vx: -3.0,
  },
  // 意地悪3: トゲの上の往復。踏みつけ不能。
  {
    x: T(120),
    y: T(7) - 24,
    w: 36,
    h: 36,
    left: T(115),
    right: T(130),
    vx: 1.8,
  },
  // 意地悪4: ゴール直前の高速番人
  {
    x: T(285),
    y: T(10) - 24,
    w: 36,
    h: 36,
    left: T(280),
    right: T(290),
    vx: -2.0,
  },
];

// =================================================================
// ユーティリティ関数
// =================================================================

/**
 * ASCIIマップ文字列をタイルIDの二次元配列に変換する
 * @param {string} str ASCII文字列のマップ
 * @returns {number[][]} タイルマップ
 */
export function parseLevel(str) {
  const rows = str.trim().split("\n");
  const h = rows.length;
  const w = Math.max(...rows.map((r) => r.length));
  return Array.from({ length: h }, (_, y) => {
    const row = rows[y].padEnd(w, ".");
    return Array.from({ length: w }, (_, x) => LEGEND[row[x]] ?? TILE_ID.Empty);
  });
}
